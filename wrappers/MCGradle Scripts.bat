...
if exist "MCGradle Scripts.ps1" (
    ...
    where /q pwsh
    IF ERRORLEVEL 1 (
        PowerShell -ExecutionPolicy Bypass -NoLogo -NoProfile -Command ". { Invoke-WebRequest -useb https://raw.githubusercontent.com/Jonathing/MCGradle-Scripts/develop/updater/update.ps1 } | Invoke-Expression"
        if exist "MCGradle Scripts.ps1" (
            DEL /f HAS_UPDATED
            PowerShell -ExecutionPolicy Bypass -NoLogo -NoProfile -File ".\MCGradle Scripts.ps1" FromCMD
        )
    ) ELSE (
        pwsh -ExecutionPolicy Bypass -NoLogo -NoProfile -Command ". { Invoke-WebRequest -useb https://raw.githubusercontent.com/Jonathing/MCGradle-Scripts/develop/updater/update.ps1 } | Invoke-Expression"
        if exist "MCGradle Scripts.ps1" (
            DEL /f HAS_UPDATED
            pwsh -ExecutionPolicy Bypass -NoLogo -NoProfile -File ".\MCGradle Scripts.ps1" FromCMD
        )
